...
export function SuggestedActions({ onActionSelect }: { onActionSelect: (action: string) => void }) {
  return (
    <div className="w-full overflow-x-auto">
      <div className="flex flex-row gap-2 items-center justify-start px-4 py-2 space-x-2">
        {actions.map((item, index) => (
          <Button
            ...
            variant="outline"
            size="sm"
            className="text-xs px-2 py-1 h-auto whitespace-nowrap bg-background/50 backdrop-blur-sm border-accent/20 hover:border-accent hover:bg-accent/10 transition-all duration-200"
          >
            {item}
          </Button>
        ))}
